...
export default function AddProduct() {
  ...
  return (
    <div className="p-10 bg-[#f8fafc] min-h-screen">
      <button
        onClick={() => navigate(-1)}
        className="flex items-center gap-2 mb-6 text-xs font-bold text-slate-400"
      >
      ...
      <form
        ...
        className="bg-white p-10 rounded-[3rem] max-w-4xl mx-auto space-y-4 shadow-xl"
      >
        ...
        />
        <textarea
          placeholder="Crop Benefits (one per line)"
          className="input h-24"
          value={form.crop_benefits}
          onChange={(e) => setForm({ ...form, crop_benefits: e.target.value })}
        />
        <textarea
          placeholder="Product Advantages (one per line)"
          className="input h-24"
          value={form.product_advantages}
          onChange={(e) => setForm({ ...form, product_advantages: e.target.value })}
        />
        <textarea
          placeholder="Recommended Crops (one per line)"
          className="input h-24"
          value={form.recommended_crops}
        ...
        >
          {loading ? <Save className="animate-spin" size={20} /> : null}
