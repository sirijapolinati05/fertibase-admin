...
export default function Resources() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      let imageUrls = {
        telugu: imagePreviews.telugu,
        marathi: imagePreviews.marathi,
        kannada: imagePreviews.kannada,
        gujarati: imagePreviews.gujarati,
      };
      ...
      if (category !== "PPTs") {
        for (const lang of ["telugu", "marathi", "kannada", "gujarati"]) {
          const newFile = files[lang];
          if (!newFile) continue; // ✅ keep old image
          ...
          imageUrls[lang] = data.publicUrl; // ✅ replace only changed image
        }
      }
      ...
      const payload = {
        ...
        image_url_telugu: imageUrls.telugu,
        image_url_marathi: imageUrls.marathi,
        image_url_kannada: imageUrls.kannada,
        image_url_gujarati: imageUrls.gujarati,

        file_url: category === "PPTs" ? fileUrl : null,
      };
  ...
  const confirmDelete = async () => {
    try {
      await supabase.from("resources").delete().eq("id", deletingId);
